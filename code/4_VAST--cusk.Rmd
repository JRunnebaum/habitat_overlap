---
title: Cusk spatial temporal model
output:
  pdf_document:
    fig_caption: yes
    number_sections: yes
...
#read in SPRING cusk data, combined longline and trawl survey data
# spring_cusk <- read.csv("../data/cusk_data_for_VAST/spring_cusk_all.csv")
fall_cusk <- read.csv("../data/cusk_data_for_VAST/fall_cusk_all.csv")

#set data here, this will change for each season 
#
# Data_Set <- spring_cusk
Data_Set <- fall_cusk

# Data_Set <- Data_Set[Data_Set$YEAR_ >= 1980 & Data_Set$YEAR_ <= 2015, ]
Data_Set$Q <- as.factor(Data_Set$survey) #set identity matrix
 
# Data_Set$Q<- as.factor(Data_Set$SURVEY_TWO)
# levels(Data_Set$Q) = c(0, 1)
...
# Default
# strata.limits <-  data.frame('stat_area'=c(462, 463, 464, 465, 466, 467, 511, 512, 513, 514, 515, 521, 522, 551, 561 ))
strata.limits <- list('Strata'= c(464, 465, 466, 467, 511, 512, 513, 514, 515, 521, 522, 525, 526, 538, 551, 552, 561, 562 ))
...
## Convergence
Print the diagnostics generated during parameter estimation, and I confirm that (1) no parameter is hitting an upper or lower bound and (2) the final gradient for each fixed-effect is close to zero. For explanation of parameters, please see `?Data_Fn`.
...
This is not the format needed for this study though....
